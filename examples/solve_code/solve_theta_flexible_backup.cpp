...
{   
    ...
    std::vector<double> coef_x_a(nx1-2, 0.0), coef_x_b(nx1-2, 0.0), coef_x_c(nx1-2, 0.0);
    std::vector<double> coef_y_a(nx1-2, 0.0), coef_y_b(nx1-2, 0.0), coef_y_c(nx1-2, 0.0);
    std::vector<double> coef_z_a(nx1-2, 0.0), coef_z_b(nx1-2, 0.0), coef_z_c(nx1-2, 0.0);
    double coef_x_a_val, coef_x_b_val, coef_x_c_val;
    double coef_y_a_val, coef_y_b_val, coef_y_c_val;
    double coef_z_a_val, coef_z_b_val, coef_z_c_val;
    ...
    for (int t_step=0; t_step<max_iter; ++t_step) {
        ...
        for (i=1; i<nx1-1; ++i) {
            ...
            coef_x_a[i-1] = (dt / 2.0 / dxdx) * ( 1.0 + (5.0/3.0) * sub.theta_x_left_index[i] + (1.0/3.0) * sub.theta_x_right_index[i] );
            coef_x_b[i-1] = (dt / 2.0 / dxdx) * (-2.0 -     (2.0) * sub.theta_x_left_index[i] -     (2.0) * sub.theta_x_right_index[i] );
            coef_x_c[i-1] = (dt / 2.0 / dxdx) * ( 1.0 + (1.0/3.0) * sub.theta_x_left_index[i] + (5.0/3.0) * sub.theta_x_right_index[i] );
        }

        for (i=0; i<nx1; ++i) {

            double coef_a_val = coef_x_a[i];
            double coef_b_val = coef_x_b[i];
            double coef_c_val = coef_x_c[i];
            ...
            for (k=0; k<nz1; ++k) {
                for (j=1; j<ny1-1; ++j) {
                    ...
                    rhs_x[jki] = (coef_c_val*theta[ijk+1] + (1.0+coef_b_val)*theta[ijk] + coef_a_val*theta[ijk-1]);
                }
            }
        }
        rhs_x_2 = MPI_Wtime();
