...
void SolveTheta::run(std::vector<double>& theta) {
    ...
    for (int t_step = 0; t_step < max_iter; ++t_step) {
        ...
        for (j = 1; j < ny1-1; ++j) {
            ...
            solver_z.solve(Azz.data(), Bzz.data(), Czz.data(), Dzz.data(), nx1-2, nz1-2);
            for (k = 1; k < nz1-1; ++k)
                for (i = 1; i < nx1-1; ++i)
        ...
        for (k = 1; k < nz1-1; ++k) {
            ...
            solver_y.solve(Ayy.data(), Byy.data(), Cyy.data(), Dyy.data(), nx1-2, ny1-2);
            for (j = 1; j < ny1-1; ++j)
                for (i = 1; i < nx1-1; ++i)
        ...
        for (k = 1; k < nz1-1; ++k) {
            ...
            solver_x.solve(Axx.data(), Bxx.data(), Cxx.data(), Dxx.data(), ny1-2, nx1-2);
            for (i = 1; i < nx1-1; ++i)
                for (j = 1; j < ny1-1; ++j)
